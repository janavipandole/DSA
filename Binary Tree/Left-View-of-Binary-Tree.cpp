...
public:
    vector<int> leftSideView(Node *root)
    {
        ...
        while (!q.empty())
        {
            ...
            if (mpp.find(y) == mpp.end())mpp[y] = node;
    ...
    void perOrder(Node *root, int level, vector<int> &ans)
    {
        if (root == NULL)return;
...
int main()
{
    ...
    Solution s;
    cout << "Binary Tree left Side View : " << endl;
    vector<int> ans = s.leftSideView(root);
    for (auto num : ans)
    {
        cout << num << " ";
    }
